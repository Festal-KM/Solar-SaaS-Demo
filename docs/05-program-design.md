...
> - **2 カラムレイアウト（UI 改修・データ層不変）**: コールタブ本体を電話番号ヘッダ（全幅）の下に **`grid lg:grid-cols-2`** で配置する。**左カラム=コール履歴**（`CustomerCallLog` 一覧 + 追加 `CallLogAddForm` + 行削除 `CallLogDeleteButton`・主領域）、**右カラム=各ステータス**（マエカク / サンキュー / ローン審査完了 / 施工完了 を縦積み・`CompactCallBlock` で密度高めに「ステータス / 希望日時 / メモ」を表示）。**マエカクブロックのみ**「次回アポ担当者 / 次回アクション（+ 次回アポ日程）」を read-only 併記（編集は商談タブのみ）。インライン編集（dirty + 保存/キャンセル + toast + `router.refresh`）・input id（`mk-*`/`ty-*`/`lc-*`/`pc-*`/`cl-*`）・ラベルは維持（E2E 互換）。schema/migration/server action/Zod/DTO は不変。
>
