...
const CONTRACT_EDIT_TRIGGERS = ["契約・金額・ローンを編集"] as const;
...
test.describe("顧客詳細『契約状況』タブ拡充 + 基本情報『契約予定情報』読み取り専用 pull", () => {
  ...
  test("契約状況タブ: 概況パネルは廃止され、案件詳細（per-contract 契約・金額・商材ライン・認定）が表示・編集できる", async ({
    ...
    // 契約サマリのポップアップ編集トリガー（契約・金額・ローンを編集）は契約タブから廃止され、
    // インライン編集に置き換わった（ローン情報の編集はローン審査タブに分離）。
    await expect(
      panel.getByRole("button", { name: "契約・金額・ローンを編集" }),
    ).toHaveCount(0);
