...
test.describe("顧客詳細『契約状況』タブ拡充 + 基本情報『契約予定情報』読み取り専用 pull", () => {
  ...
  test("基本情報タブ『契約予定情報』: 契約予定情報が読み取り専用で表示され、契約系の編集トリガーが描画されない", async ({
    ...
    // 「契約予定情報」区分見出しの下に契約予定情報が pull 表示される。
    await expect(
      panel.getByRole("heading", { name: "契約予定情報", exact: true }),
    ).toBeVisible();
    ...
    // 一方で、現状情報側の編集（顧客情報インライン編集 / 概況編集 / ヒアリング編集）は
    // 引き続き基本情報タブに残る（契約系のみが契約状況タブへ移動した）。
    await expect(
      panel.getByRole("button", { name: "概況を編集" }).first(),
    ).toBeVisible();
  });
