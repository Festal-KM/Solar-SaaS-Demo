...
// 顧客詳細「契約状況」タブ + 基本情報タブ「契約予定情報」読み取り専用 pull +
// 顧客情報インライン編集からの住所(フルテキスト)欄削除 を検証する
// (F-031 / F-062 / docs/02 顧客詳細・契約予定情報ユースケース)。
//
// 今回の変更点（検証対象）:
//   1. 契約状況タブ(value="contract"): 概況(ContractStatusPanel)は廃止。
//      「契約予定情報（案件詳細）（ProjectContractList editable: 契約概要・金額・契約
//      予定日・契約番号・商材ライン・特記事項 を per-contract で表示・編集）」の単一面。
//      商材ラインはカード内インライン編集（customer-contract-equipment.spec で詳細検証）、
//      契約サマリ（契約日・支払・入金日・シリアル）もカード内インライン編集。ローン情報の
//      編集はローン審査タブの EditContractDialog に分離（契約タブからポップアップは廃止）。
//      サブタブヘッダに「契約を追加」+「契約を削除」を並置（ゴミ箱アイコン行は廃止）。
//   2. 基本情報タブの「契約予定情報」: 契約状況タブと同じ内容を読み取り専用 pull
//      (contractReadOnly)。契約系のインライン編集（#ct-date / 設備明細を編集）が一切
//      描画されない。
...
test.describe("顧客詳細『契約状況』タブ拡充 + 基本情報『契約予定情報』読み取り専用 pull", () => {
  ...
  test("契約状況タブ: 概況パネルは廃止され、案件詳細（per-contract 契約・金額・商材ライン・認定）が表示・編集できる", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await openContractedCustomer(page);

    // 「契約状況」タブへ切替。契約サブタブ（契約 #1 / #2 …）を内包するため、
    // 外側パネル（id 末尾 -content-contract）にスコープして tabpanel の曖昧さを排除する。
    await page.getByRole("tab", { name: "契約" }).first().click();
    const panel = page.locator('[role="tabpanel"][id$="-content-contract"]');
    await expect(panel).toBeVisible();

    // 概況セクション（ContractStatusPanel: 契約プラン/金額/予定日）は廃止されている。
    ...
    // 案件詳細セクション（ProjectContractList editable）: 契約・金額サマリ + 契約サブタブ。
    await expect(
      panel.getByRole("heading", { name: "契約予定情報（案件詳細）" }),
    ).toBeVisible();
    // 契約・金額サマリ見出し（全契約合計）+ 契約サブタブ（契約 #1）が併存する（再設計）。
    await expect(panel.getByRole("heading", { name: "契約・金額", exact: true }).first()).toBeVisible();
    await expect(page.getByRole("tab", { name: /^契約 #1$/ })).toBeVisible();

    // per-contract の契約サマリはカード内インライン編集化された（ポップアップ廃止）。
    // 契約日入力 #ct-date と契約金額（商材ライン合計・read-only ラベル）が描画される。
    await expect(panel.locator("#ct-date").first()).toBeVisible();
    // 契約金額は商材ライン合計の自動表示（手動入力は廃止・要件B）。
    await expect(panel.getByText("契約金額（商材ライン合計）").first()).toBeVisible();
    // 商材ラインはカード内インライン編集（PV 金額欄が描画される）。
    await expect(panel.locator("#eq-amount-PV").first()).toBeVisible();
    await expect(panel.getByRole("heading", { name: "特記事項" }).first()).toBeVisible();

    // 契約サマリのポップアップ編集ダイアログは廃止され、カード内インライン編集
    // （#ct-date 等）に置き換わった。ローン情報は独立 LoanReview としてローン審査タブへ分離。
    await expect(
      panel.getByRole("button", { name: /契約.*編集/ }),
    ).toHaveCount(0);

    // サブタブヘッダに「契約を追加」と「契約を削除」が並ぶ（ゴミ箱アイコン行は廃止）。
    ...
    // 契約サマリのインライン編集→保存→反映: 契約日を書き換え、保存ボタンで dirty 連動保存。
    const newDate = "2030-09-15";
    await panel.locator("#ct-date").first().fill(newDate);
    await panel.getByRole("button", { name: "保存" }).first().click();
    // 保存後 router.refresh で再描画され、入力値が保持される。
  ...
  test("基本情報タブ『契約予定情報』: 契約予定情報が読み取り専用で表示され、契約系の編集トリガーが描画されない", async ({
    ...
    // 契約サマリのインライン編集（#ct-date 等）は基本情報タブの読み取り専用 pull には
    // 描画されない（編集面は契約状況タブに集約）。
    await expect(
      panel.locator("#ct-date"),
      "基本情報タブの読み取り専用 pull に契約サマリのインライン編集（#ct-date）が描画されないこと",
    ).toHaveCount(0);
