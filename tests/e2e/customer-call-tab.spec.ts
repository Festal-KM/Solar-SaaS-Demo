...
test.describe("顧客詳細 コールタブ（4 セクション・インライン編集）", () => {
  ...
  test("シナリオ2a: 履歴なし顧客はマエカクコールに「履歴はありません」が表示される", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await openCustomerByName(page, CUSTOMER_NO_HISTORY);
    ...
    // マエカクコールセクション内に過去コール履歴見出しが存在する。
    await expect(panel.getByRole("heading", { name: "過去コール履歴" }).first()).toBeVisible();
    // 佐藤 一馬 (withPreCall:false) → 履歴なしメッセージ。
    await expect(panel.getByText("履歴はありません").first()).toBeVisible();
  });
  ...
  test("シナリオ2b: 履歴あり顧客はマエカクコールに過去コール履歴行が表示される", async ({
    ...
    // 過去コール履歴見出し + 履歴行（PreCall 由来のラベル）が描画される。
    await expect(panel.getByRole("heading", { name: "過去コール履歴" }).first()).toBeVisible();
    // 履歴なしメッセージは出ない。
    await expect(panel.getByText("履歴はありません")).toHaveCount(0);
    // PreCall 行内の代表ラベル（架電日時 / 結果）が存在する。
    await expect(panel.locator("dt", { hasText: "架電日時" }).first()).toBeVisible();
    await expect(panel.locator("dt", { hasText: "結果" }).first()).toBeVisible();
  });
