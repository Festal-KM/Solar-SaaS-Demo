...
test.describe("顧客詳細 コールタブ（4 セクション・インライン編集）", () => {
  ...
  test("シナリオ3: サンキューコールをインライン編集 → 保存 → リロード後も永続化される", async ({
    ...
    // dirty によりサンキューコールブロックの保存ボタンが活性化する。
    // 2 カラム化でステータスは右カラム内の CompactCallBlock(div) になったため、
    // 旧 section スコープ（右カラム section 全体に 4 ブロックの保存が同居 → strict 違反）を廃し、
    // 見出し <h4>サンキューコール</h4> を持つブロック div（h4 の親 div）に直接スコープする。
    const block = panel
      .getByRole("heading", { name: "サンキューコール", exact: true })
      .first()
      .locator("xpath=..");
    const saveBtn = block.getByRole("button", { name: "保存" });
  ...
  test("シナリオ4: マエカク希望日時が商談履歴タブのマエカク希望日時と同一列を共用する", async ({
    ...
    // 2 カラム化でマエカクコールも右カラム内 CompactCallBlock(div) になったため、
    // 見出し <h4>マエカクコール</h4> を持つブロック div（h4 の親 div）にスコープする。
    const block = panel
      .getByRole("heading", { name: "マエカクコール", exact: true })
      .first()
      .locator("xpath=..");
    const saveBtn = block.getByRole("button", { name: "保存" });
    await expect(saveBtn).toBeEnabled();
