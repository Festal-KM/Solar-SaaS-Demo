...
test.describe("F-031 顧客詳細『基本情報』タブ インライン編集再設計", () => {
  ...
  test("レイアウト: 現状情報 / 契約予定情報 の区分見出しと既存設備・契約予定が正しい側に描画される", async ({
    ...
    // 区分見出し（SectionHeading）が両方描画される。
    await expect(panel.getByRole("heading", { name: "現状情報", exact: true })).toBeVisible();
    await expect(panel.getByRole("heading", { name: "契約予定情報", exact: true })).toBeVisible();

    // 既存設備カードが現状情報側に描画され、3 カテゴリの有無が見える。
    // 同カテゴリは embedded「既設設備（現況）」にも描画されるため .first() で限定する。
    await expect(panel.getByRole("heading", { name: "既存設備", exact: true })).toBeVisible();
