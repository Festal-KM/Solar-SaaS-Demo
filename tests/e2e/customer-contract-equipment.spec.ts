...
async function openContractTab(page: Page): Promise<Locator> {
  ...
  // 契約タブ配下は契約サブタブ（契約 #1 / #2 …）を内包するため、role=tabpanel が
  // 外側（契約）+ 内側（契約 #N）の 2 つ active になる。外側パネル（id 末尾 -content-contract）に
  // スコープして曖昧さを排除する。商材ライン等の内側要素も外側パネルの子孫として参照できる。
...
// 金額入力は MoneyInput により ¥ + カンマ整形表示される（保存値は整数のまま）。
...
test.describe("契約状況タブ インライン編集 + 商材金額 + 施工ライン + 契約ファイル + 契約自動作成", () => {
  ...
  test("契約あり顧客: 概況パネルが無く、PV/BT/付帯/施工の商材カードがインライン編集フォーム＋金額欄で表示される", async ({
    ...
    // (#2/#3) 商材カードがインライン編集フォームとして描画される。PV/BT/施工が
    // それぞれ金額入力欄(#eq-amount-<cat>)を持つ。ポップアップ起動の鉛筆/追加(＋)トリガーは
    // インライン化により出ない。
    // ※ 改修#3 で付帯商材(ACCESSORY)は単一カード(#eq-amount-ACCESSORY)から
    //   「複数行 + 付帯商材を追加」運用へ変更された。そのため ACCESSORY は
    //   #eq-amount-ACCESSORY を持たず、専用セクション見出し + 追加導線で検証する。
    for (const cat of ["PV", "BT", "CONSTRUCTION"]) {
      await expect(panel.locator(`#eq-amount-${cat}`).first(), `${cat} 金額欄`).toBeVisible();
    }
    // 旧 単一付帯カードの金額欄は廃止されている。
    await expect(
      panel.locator("#eq-amount-ACCESSORY"),
      "付帯商材は単一カード金額欄ではなく複数行運用へ移行",
    ).toHaveCount(0);
    // 付帯商材セクション（複数行運用）の「付帯商材を追加」導線が描画される。
    await expect(panel.getByRole("button", { name: "付帯商材を追加" })).toBeVisible();
  ...
  test("契約あり顧客: PV の金額・メーカーをインライン編集→保存→反映。施工(CONSTRUCTION)の金額・業者も保存→反映", async ({
    ...
    // （契約金額は商材ライン amount の合計をサーバーが再計算する＝改修#2。ここでは
    //  商材ラインの金額入力値の永続化で検証し、合計再計算は revamp spec で検証する。）
    await expect(panel.locator("#eq-maker-PV").first()).toHaveValue(pvMaker, { timeout: 30_000 });
    await expect(panel.locator("#eq-amount-PV").first()).toHaveValue(yen(pvAmount), {
      timeout: 30_000,
    });
    ...
    await expect(panel.locator("#eq-amount-CONSTRUCTION").first()).toHaveValue(yen(conAmount), {
      timeout: 30_000,
    });
  });
  ...
  test("契約なし顧客（佐藤 一馬）: 商材金額入力→保存で契約が自動作成され、再保存しても契約が二重作成されない", async ({
    ...
    if (startedClean) {
      ...
      // 契約サブタブ「契約 #1」はまだ無い（契約自動作成前）。
      await expect(page.getByRole("tab", { name: /^契約 #1$/ })).toHaveCount(0);
      ...
      // 契約が自動作成され、契約サブタブ「契約 #1」+ PV メーカー + 金額が描画される。
      ...
      // 再実行（既に契約自動作成済み）: 契約サブタブは既に 1 件。
      await expect(page.getByRole("tab", { name: /^契約 #1$/ })).toHaveCount(1);
    }
    ...
    // 契約サブタブは依然 1 件のみ（#2 が生成されていない = 二重作成なし）。
    await expect(page.getByRole("tab", { name: /^契約 #1$/ })).toHaveCount(1);
    await expect(page.getByRole("tab", { name: /^契約 #2$/ })).toHaveCount(0);
