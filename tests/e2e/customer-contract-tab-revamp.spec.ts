...
test.describe("顧客詳細『契約』タブ 4 改修（架電削除 / 金額自動 / 付帯複数 / 契約サブタブ）", () => {
  ...
  test("改修#1: EditContractDialog から架電(#ct-call/#ct-loancall)・契約金額手動(#ct-amount)が消え、ローン情報は残存", async ({
    ...
    // 契約サマリのポップアップ編集は契約タブから廃止（インライン化）。ローン情報の編集ダイアログは
    // ローン審査タブの「契約・金額・ローンを編集」トリガーに分離されたため、ローン審査タブで開く。
    await page.getByRole("tab", { name: "ローン審査" }).click();
    const panel = page.locator('[role="tabpanel"][id$="-content-loan"]');
    await expect(panel).toBeVisible();

    // 契約・金額・ローンの編集ダイアログを開く（ローン審査タブ内のトリガー）。
