...
//
// 検証対象（契約タブの 4 改修）:
//   1. 編集ダイアログ廃止 + 架電/金額手動の削除: 契約サマリのポップアップ編集ダイアログ
//      （EditContractDialog）は廃止され、契約メタはカード内インライン編集
//      （ContractDetailInlineEdit: #ct-date / #ct-serial）へ移行。「コールステータス(#ct-call)」
//      「ローン審査架電日時(#ct-loancall)」「契約金額の手動入力(#ct-amount)」は消滅。
//      ローン情報（#ct-loanreview/#ct-loanco 等）は独立 LoanReview のローン審査タブへ分離。
...
//      サブタブ（契約 #1 / 契約 #2 …）になる。特記事項と契約ファイルは契約タブ直下（共有）。
//
...
// MoneyInput は ¥ + カンマ整形表示（保存値は整数のまま）。
function yen(n: number): string {
  return `¥${n.toLocaleString("ja-JP")}`;
}

// per-contract「契約金額（商材ライン合計）」の表示値（¥…）を数値で読む。
// 卸業者（編集可）の契約サブタブでは商材ライン合計が <label>契約金額（商材ライン合計）</label>
// + 直後の <p>¥…</p>（自動計算・read-only）として描画される（ContractDetailInlineEdit）。
// label の次の sibling p を読む（dealer/read-only の MetaItem dt/dd は editable 画面には出ない）。
async function contractAmountTotal(panel: Locator): Promise<number> {
  const label = panel.locator("label", { hasText: "契約金額（商材ライン合計）" }).first();
  const p = label.locator("xpath=following-sibling::p[1]");
...
test.describe("顧客詳細『契約』タブ 4 改修（架電削除 / 金額自動 / 付帯複数 / 契約サブタブ）", () => {
  ...
  test("改修#1: 契約サマリのポップアップ編集ダイアログは廃止され、契約メタはカード内インライン編集（架電/金額手動/ローン入力なし）", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await openContractedCustomer(page);

    // 契約サマリの編集はポップアップダイアログ（EditContractDialog）を廃止しカード内
    // インライン編集（ContractDetailInlineEdit）へ移行。ローン情報は独立 LoanReview として
    // ローン審査タブへ分離（contract のローン列は UI からの編集導線が消滅）。
    const panel = await openContractTab(page);
    await expect(page.getByRole("tab", { name: /^契約 #1$/ })).toBeVisible({ timeout: 30_000 });

    // ポップアップ編集トリガー（契約…編集ダイアログ）は契約タブから消滅している。
    await expect(
      panel.getByRole("button", { name: /契約.*編集/ }),
      "契約サマリのポップアップ編集トリガーが廃止されていること",
    ).toHaveCount(0);

    // 契約メタはカード内インライン編集（#ct-date / #ct-serial）が描画される。
    await expect(panel.locator("#ct-date").first(), "契約日のインライン編集は残存").toBeVisible();
    await expect(panel.locator("#ct-serial").first(), "シリアルのインライン編集は残存").toBeVisible();

    // (改修#1) 架電関連・契約金額手動入力は一切無い。
    await expect(panel.locator("#ct-call"), "コールステータス入力が削除されていること").toHaveCount(0);
    await expect(
      panel.locator("#ct-loancall"),
      "ローン審査架電日時入力が削除されていること",
    ).toHaveCount(0);
    await expect(
      panel.locator("#ct-amount"),
      "契約金額の手動入力が削除されていること",
    ).toHaveCount(0);

    // ローン情報入力（会社/審査ステータス/頭金/団信/備考）は契約タブのインライン編集から
    // 完全に消滅している（独立 LoanReview のローン審査タブへ移行）。
    for (const id of ["#ct-loanco", "#ct-loanreview", "#ct-down", "#ct-credit", "#ct-loannote"]) {
      await expect(
        panel.locator(id),
        `契約タブのインライン編集にローン入力 ${id} が描画されないこと`,
      ).toHaveCount(0);
    }

    // 契約日をインライン編集→保存→反映（契約メタ編集の動作確認）。
    const newDate = "2030-11-20";
    await panel.locator("#ct-date").first().fill(newDate);
    await panel.getByRole("button", { name: "保存" }).first().click();
    await expect(panel.locator("#ct-date").first()).toHaveValue(newDate, { timeout: 30_000 });
  });
  ...
  test("改修#2: PV 金額をインライン編集→保存で契約金額(summary + per-contract)が当該合計へ再計算される", async ({
    ...
    await pvSave.click();
    await expect(pvInput).toHaveValue(yen(newPv), { timeout: 30_000 });
    ...
    await expect(pvInput).toHaveValue(yen(newPv2), { timeout: 30_000 });
  ...
  test("改修#4: 「契約を追加」で契約#2 サブタブが出現し、サブタブを切替できる。認定・契約ファイルは共有", async ({
    ...
    // 特記事項（フリーテキストメモ）と契約関連ファイルは契約タブ直下（サブタブ外・共有）。
  ...
  test("回帰: 二次店物理除外/損益タブ非表示が壊れていない（卸業者には損益タブ・契約金額が見える）", async ({
    ...
    // 契約があれば原価系列（施工代）ヘッダが描画される（二次店 DTO では物理除外。
    // 卸業者ではキー存在）。0 契約のときは空状態のためヘッダ非描画。
    const constructionFeeHeader = profitPanel.getByText("施工代").first();
    if ((await constructionFeeHeader.count()) > 0) {
      await expect(constructionFeeHeader).toBeVisible();
    }
  });
});
