...
//      行削除も。
//   5. 不備（専用フォーム）: 「不備内容・解消状況」セクションの専用フォームで不備内容＋担当者を
//      登録 → 不備一覧に未解消で出現（担当者名表示）→ 解消セレクトで解消済み → リロード保持。
//
...
test.describe("ローン審査タブ — 独立 LoanReview の CRUD・履歴・不備", () => {
  ...
  test("過去の審査履歴ログを追加（日時+結果+メモ）→ 一覧反映 → 行削除", async ({ page }) => {
    ...
    // 追加フォーム: 日時 + 結果(可決) + メモ。
    await panel.locator(`#lrl-at-${lrId}`).fill("2026-06-21");
  ...
  test("不備（専用フォーム）: 不備内容＋担当者を登録 → 不備一覧に未解消で出現（担当者名表示）→ 解消トグル", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await openCustomerByQuery(page, EMPTY_CUSTOMER_QUERY);
    ...
    // 「不備内容・解消状況」セクションの専用フォームで不備内容＋担当者を登録。
    const defectMarker = `E2E不備-${Date.now()}`;
    await panel.locator(`#lrl-defect-${lrId}`).fill(defectMarker);
    await panel.locator(`#lrl-defect-at-${lrId}`).fill("2026-06-22");
    // 担当者 select の 2 番目（先頭は「未割当」）を選び、表示名を控える。
    const assigneeSelect = panel.locator(`#lrl-defect-assignee-${lrId}`);
    await assigneeSelect.selectOption({ index: 1 });
    const assigneeName = (
      await assigneeSelect.locator("option:checked").textContent()
    )!.trim();
    await panel.getByRole("button", { name: "不備を追加" }).first().click();
    await expect(page.getByText("保存しました").first()).toBeVisible({ timeout: 30_000 });

    // 「不備内容・解消状況」一覧に不備内容が未解消 + 担当者名付きで出現。
    // 不備は result=defect の LoanReviewLog として保存されるため、同じ marker が
    // 審査履歴ログ一覧の <li> にも現れる。可変セレクト(id=lrl-defstatus-*)を持つ
    // 行 = サマリ行に scope する（履歴行は select を持たない）。
    // 可変セレクトの値=open を未解消の根拠とする
    //（select は両ラベルを常時 DOM に持つため toContainText では状態判定できない）。
    const defectRow = panel
      .locator("li", { hasText: defectMarker })
      .filter({ has: page.locator('select[id^="lrl-defstatus-"]') });
    await expect(defectRow).toBeVisible({ timeout: 30_000 });
    const defStatus = defectRow.locator('select[id^="lrl-defstatus-"]');
    await expect(defStatus).toHaveValue("open", { timeout: 30_000 });
    await expect(defStatus.locator('option[value="open"]')).toHaveText("未解消");
    await expect(defStatus.locator('option[value="resolved"]')).toHaveText("解消済み");
    await expect(defectRow).toContainText(assigneeName);

    // 解消ステータス（可変セレクト）→ 解消済みへ。
    await defStatus.selectOption("resolved");
    await expect(page.getByText("保存しました").first()).toBeVisible({ timeout: 30_000 });
    await expect(defectRow.locator('select[id^="lrl-defstatus-"]')).toHaveValue("resolved", {
      timeout: 30_000,
    });

    // リロード後も解消済みを保持（select の value で厳密に検証）。
    await page.reload();
    panel = await openLoanTab(page);
    await expect(
      panel.locator("li", { hasText: defectMarker }).locator('select[id^="lrl-defstatus-"]'),
    ).toHaveValue("resolved", { timeout: 30_000 });
