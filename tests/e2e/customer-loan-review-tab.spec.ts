...
const DEFECT_STATUS_LABELS = ["なし", "不備あり（未解消）", "解消済み"] as const;
...
test.describe("ローン審査タブ — 独立 LoanReview の CRUD・履歴・不備", () => {
  ...
  test("インライン編集（ステータス/会社/頭金/不備内容/解消ステータス/審査日）を保存しリロード後も保持", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await openCustomerByQuery(page, EMPTY_CUSTOMER_QUERY);
    ...
    await panel.locator(`#lr-defcontent-${lrId}`).fill("収入証明の不足");
    await panel.locator(`#lr-defstatus-${lrId}`).selectOption("defect");
    ...
    await expect(panel.locator(`#lr-defcontent-${lrId}`)).toHaveValue("収入証明の不足");
    await expect(panel.locator(`#lr-defstatus-${lrId}`)).toHaveValue("defect");
    await expect(panel.locator(`#lr-at-${lrId}`)).toHaveValue("2026-06-20");

    // 解消ステータス select に 3 値がそろっている。
    const defectOpts = (await panel.locator(`#lr-defstatus-${lrId}`).locator("option").allTextContents())
      .map((t) => t.trim());
    for (const label of DEFECT_STATUS_LABELS) {
      expect(defectOpts, `解消ステータス select に「${label}」`).toContain(label);
    }
  ...
  test("不備: 解消ステータス=不備あり（未解消）を保存 → 反映される", async ({ page }) => {
    await signInAsDemo(page);
    await openCustomerByQuery(page, EMPTY_CUSTOMER_QUERY);
    let panel = await ensureAtLeastOneReview(page);
    ...
    await panel.locator(`#lr-defstatus-${lrId}`).selectOption({ label: "不備あり（未解消）" });
    await panel.getByRole("button", { name: "保存" }).first().click();
    await expect(page.getByText("保存しました").first()).toBeVisible({ timeout: 30_000 });

    await page.reload();
    panel = await openLoanTab(page);
    await expect(panel.locator(`#lr-defstatus-${lrId}`)).toHaveValue("defect", { timeout: 30_000 });
